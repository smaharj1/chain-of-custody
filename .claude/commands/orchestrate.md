...
**Signal → status projection** (per `docs/design/loop-engineering.md` §6.1). Every hard-stop that concerns a specific item **sets that item's `status: blocked` + `blockReason`** before pausing — a hard-stopped item must never be left `in_progress` (a resume would silently re-run it into the same wall):
...
### 5.5 Primer delta
If this item introduced a new pattern (first WebSocket, first background job, new module layout), update the relevant `.claude/context/` primer + exemplar pointer **now** — same contract as Tech-Lead A.8.5. The next item's autonomous Architect designs from these primers; over a long run, stale primers are silent design drift.

### 6. Update + log
Regenerate `BUILD_PLAN.md`; re-stamp the lock `heartbeat`; append a `.claude/loop.jsonl` record `{ts, type: "item", itemId, iteration, status, blockReason, breaker, verifyEvidence, tool_calls, securityVerdict}` (`breaker: true` iff this record is a `blocked` landing that incremented the circuit breaker — the re-derivation in preamble 6.6 reads it); refresh the progress surface: **current item, items done/total, blocked count, tool-calls used vs `max_tool_calls_per_run` (the measurable cap), token estimate (best-effort, label it as an estimate — never a precise figure), last verify result.**
