...
4. **Primer-readiness check**: grep primers + `CLAUDE.md` for template sentinels (`<!--` placeholder comments, `TODO(primer)`), skipping the generic protocol files (`engineer-protocol.md`, `primer-protocol.md`, which quote the sentinels by design). If context the run needs is still a template, **derive it** per `.claude/context/primer-protocol.md` — primers are agent-written, never a user chore — then re-grep; only surviving `TODO(primer)` markers **downgrade autonomy to `per-feature`**, with the reason reported.
...
6. **Update** plan + regenerate `BUILD_PLAN.md` + re-stamp the lock heartbeat + append `loop.jsonl` (including `primersUpdated`) + refresh the progress surface (§13). Then the **primer delta**: apply each engineer report's `## Primer Delta` and the reviewer's `## Primer Staleness`, plus any new pattern they missed — primer + exemplar pointer, now — and close out `TODO(primer)` markers this item resolved (the only thing that lifts the §6 autonomy downgrade). The next item's autonomous Architect designs from these primers with no human reading over its shoulder, so a skipped delta compounds across the run.
...
**Autonomy precondition**: `per-milestone`/`unattended` require non-placeholder primers (detected by the §6 sentinel grep, after the preamble's derivation pass). Otherwise the run downgrades to `per-feature`.
...
- **Neither gate is a user chore.** Both gates originally read "tell the user which files to fill in," which assumed a user who could answer "which ORM?" and "what's your test command?" — the wrong assumption for this kit's audience. Primers and the execution-critical commands are **agent-derived** (`.claude/context/primer-protocol.md`): the Planner writes them as a plan output, the Orchestrator's preamble derives what a run needs, and the Architect/Tech Lead keep them true as patterns land. The gates remain as *readiness checks on the derivation*, not as instructions to a human.
