...
max_retries_per_item: 2          # within-item verify retries before the item is marked `blocked`
...
security_review: sensitive       # sensitive (default) | every_item | off
                                 ...
                                 # does NOT reach `done` until the findings are resolved or waived.
...
enforce_engineer_git_hook: true  # if true, expects a PreToolUse hook blocking git-write Bash for
                                 # engineer subagents (see .claude/hooks/). If false, you accept the
                                 # residual risk that an engineer subagent could run a git write.
```
...
- **Capabilities the loop expects.** A screenshot tool for auto-demo (e.g. the `Claude_Preview` MCP) and a notification path for checkpoints (e.g. a `PushNotification` tool). Both have text fallbacks if absent (route + HTTP check; transcript banner) — see `orchestrate.md` Run Preamble.
